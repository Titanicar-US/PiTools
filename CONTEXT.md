...
- `make check` passed after the exact-head security and documentation updates on 2026-08-10: Rust formatting, Clippy with warnings denied, the locked Rust test suite, 23 BDD scenarios (68 steps passed), and 18 Pi worker tests. The focused contracts cover bounded Actions logs, check-run annotations, envelope redaction, oversize rejection, canonical path rejection, typed CI approval admission, protected operator routes, CLI command discovery, review-thread feedback replies, issue-comment outcome rendering, and waiting-approval Skip/Cancel behavior.
