...
- `make check` passed after the exact-head security and documentation updates on 2026-08-10: Rust formatting, Clippy with warnings denied, the locked Rust test suite, 29 BDD scenarios (92 steps passed), and 18 Pi worker tests. The focused contracts cover bounded Actions logs, check-run annotations, envelope redaction, oversize rejection, canonical path rejection, typed CI approval admission, protected operator routes, CLI command discovery, review-thread feedback replies, issue-comment outcome rendering, waiting-approval Skip/Cancel behavior, and advisory readiness gates for conflicts, stale branches, failed or pending checks, missing approvals, unresolved automation feedback, and fully ready PRs.
- Code-bearing validation head `0a4390caf74e48da9a10f1721cd87f26e4a92f8c` passed the exact-head [source workflow](https://github.com/Titanicar-US/PiTools/actions/runs/31393394755) and [core/runner image workflow](https://github.com/Titanicar-US/PiTools/actions/runs/31393394709). The source check, core image, and runner image are green; pull-request image publication remains intentionally skipped because PR #1 is draft, and the post-merge publisher is release-tag-only.
...
- Ephemeral local Compose acceptance passed with non-production credentials using `PITOOLS_ACCEPTANCE_HTTP_PORT=18081 PITOOLS_ACCEPTANCE_POSTGRES_PORT=15433 PITOOLS_ACCEPTANCE_NATS_PORT=14223 make acceptance`: PostgreSQL and NATS became healthy, `/healthz` and `/readyz` returned success, unauthenticated operator access returned 401, a signed PR webhook returned 202, its duplicate returned 208, and watchlist/event persistence plus webhook metrics were observed. The unique Compose project and temporary credentials were removed after the run. The default loopback ports were occupied by another Compose project, so the documented port overrides were required for this run.
