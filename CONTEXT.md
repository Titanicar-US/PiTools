...
- Published validation head `fb5e4834b7b0810645d3e40a43b2ac842c30f126` passed the [source workflow](https://github.com/Titanicar-US/PiTools/actions/runs/31384541757) and [core/runner image workflow](https://github.com/Titanicar-US/PiTools/actions/runs/31384541826). The source workflow had no repository-owned warnings; the image workflow passed with upstream Docker action Node 20 runtime and BuildKit no-output warnings, and its release-only publication job was skipped as designed. The next image-validation change uses the runner Docker CLI for pull-request builds while retaining the protected BuildKit publisher for release tags.
