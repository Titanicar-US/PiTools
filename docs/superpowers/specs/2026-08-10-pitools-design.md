...
The Rust service owns HTTP ingress, GitHub App authentication, webhook verification, event normalization, Postgres persistence, job leases, readiness calculation, policy enforcement, GitHub API writes, comment/check-run rendering, and audit records. The CLI calls the same authenticated API and offers installation validation, watchlist inspection, reconciliation, job control, and diagnostics.
